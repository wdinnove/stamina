...
/** La vignette de l'exercice : son premier schéma, ou un cadre vide s'il n'en a pas encore. */
function Cover({ exercise }: { exercise: Exercise }) {
  if (exercise.coverScene && exercise.coverScene.elements.length > 0) {
    return <DiagramThumb scene={exercise.coverScene} radius={6} style={{ width: 60, maxWidth: 60 }} />;
  }
  return (
    <div style={{
...
export default function ExercisesPage() {
  ...
  const filtered = exercises.filter(ex =>
    ...
    (
      ...
      (ex.categoryName ?? '').toLowerCase().includes(needle)
    )
  );
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, gap: 12, flexWrap: 'wrap' }}>
        ...
        {canEditTeamData && (
          <AddButton label="Ajouter un exercice" onClick={() => navigate('/exercices/nouveau')} />
        )}
      </div>
      ...
      {!loading && filtered.length === 0 && (
        canEditTeamData ? (
          <DropzoneEmptyState
            ...
            onClick={search ? undefined : () => navigate('/exercices/nouveau')}
          />
        ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', minWidth: 360 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #2A2F3A' }}>
                ...
                <th className="hidden sm:table-cell sm:px-5 sm:w-[70px]" style={TH}>Vidéo</th>

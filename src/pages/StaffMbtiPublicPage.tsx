...
/** Pendant de `PlayerMbtiPublicPage` pour un membre du staff — même questionnaire, même moteur
 *  de calcul, seule la source des réponses change (`staff_id` au lieu de `player_id`). */
export default function StaffMbtiPublicPage() {
  ...
  useEffect(() => {
    ...
    mbtiApi.getStaffPublicInfo(staffId)
      .then(info => {
        if (!info) { setNotFound(true); }
        else {
          setStaffName(playerNameFull(info));
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!staffId || !complete) return;
    setStatus('submitting');

    const { error } = await mbtiApi.submitStaffPublic(staffId, answers);
      ...
      <Page>
        <div style={card}>
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <p style={{ color: '#F1F5F9', fontWeight: 600, margin: '0 0 8px' }}>Membre du staff introuvable</p>
            <p style={{ color: '#475569', fontSize: '0.82rem', margin: 0 }}>Ce lien n'est pas valide ou la personne n'existe plus.</p>
          </div>
        </div>
      </Page>
    );
        ...
        <div style={card}>
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <p style={{ color: '#475569', fontSize: '0.85rem', margin: 0 }}>
              Tes réponses sont enregistrées. Ton profil est consultable par l'équipe.
            </p>
          </div>
        </div>
      </Page>
        ...
        <div style={card}>
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <p style={{ color: '#475569', fontSize: '0.82rem', margin: 0 }}>
              Il ne se remplit qu'une fois. Si tu penses t'être trompé, demande à un admin de le réinitialiser.
            </p>
          </div>
        </div>
      </Page>
    ...
    <Page>
      <div style={card}>
        <div style={{ marginBottom: 20 }}>
          ...
          <h1 style={{ color: '#F1F5F9', fontWeight: 700, fontSize: '1.15rem', margin: 0 }}>{staffName}</h1>
        </div>
        ...
        <div style={{ padding: '12px 14px', backgroundColor: '#1E2229', border: '1px solid #2A2F3A', borderLeft: `3px solid ${ACCENT}`, borderRadius: 8, marginBottom: 20 }}>
          <p style={{ color: '#94A3B8', fontSize: '0.78rem', lineHeight: 1.6, margin: 0 }}>
            ...
            fonctionnement, ce n'est ni un test psychologique ni une évaluation professionnelle.
            <br />

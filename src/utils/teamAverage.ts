...
/**
 * Chiffre d'équipe : la valeur, et le nombre de joueuses qui la composent.
 *
 * `players` n'est pas décoratif — il doit être affiché à côté de la valeur. Une moyenne non
 * pondérée devient nerveuse quand peu de joueuses ont saisi (une seule saisie pèse alors autant
 ...
 * **moyenne NON PONDÉRÉE des valeurs individuelles**. On agrège d'abord par joueuse, puis on
 * moyenne les joueuses — jamais une moyenne à plat des lignes brutes.
 *
 * Pourquoi : une moyenne à plat pondère le résultat par l'assiduité. Deux blessées qui arrêtent
 * de loguer feraient monter le RPE moyen d'équipe sans que personne se soit entraîné plus dur ;
 * des joueuses qui vont mal et saisissent moins feraient monter le score de bien-être. Comme
 ...
 *
 * `valueOf` reçoit toutes les lignes d'UNE joueuse et renvoie sa valeur individuelle, ou `null`
 * si elle n'en a pas — elle sort alors du calcul ET du décompte `players`.
 *
 ...
 * par joueuse et la réconciliation par temps dès que l'assiduité n'est pas uniforme (voir
...
/**
 * Cas courant : la valeur d'une joueuse est la moyenne d'un champ numérique de ses lignes.

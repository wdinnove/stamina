...
export interface ListPlayersFilters {
  status?: PlayerStatus;
  /** Inclut les joueurs partis du club (`leftDate` renseigné) — par défaut absents : la plupart
   *  des appelants s'en servent comme vivier (partenaires, ajout à une saison), où un joueur
   *  parti n'a rien à faire. Seuls les écrans de gestion du club (Joueurs, Club) le passent. */
  includeLeft?: boolean;
}
...
export const playersApi = {
  async list(filters: ListPlayersFilters = {}): Promise<Player[]> {
    ...
    if (!filters.includeLeft) query = query.is('left_date', null);
    const { data, error } = await query.order('last_name');
    if (error) throw error;
    return (data ?? []).map(toPlayer);
  },

  /** Marque (ou efface, avec `date: null`) la date de départ du club — n'affecte ni l'effectif
   *  des saisons déjà rattachées, ni l'historique : seuls les viviers org-wide (`list` par
   *  défaut) et les nouveaux rattachements de saison cessent de le proposer. */
  async setLeftDate(playerId: string, date: string | null): Promise<void> {
    const { error } = await supabase.from('players').update({ left_date: date }).eq('id', playerId);
    if (error) throw error;
  },
...
function toPlayer(row: Record<string, unknown>): Player {
  return {
    ...
    leftDate:          row.left_date          as string | undefined,
